...
public class Ohohoh : MonoBehaviour {
    ...
    // object.
    public static SkinnedMeshRenderer UpdateCollisionMesh(GameObject model)
    {
        SkinnedMeshRenderer rend = model.GetComponent<SkinnedMeshRenderer>();
        Mesh baseMesh = rend.sharedMesh;
        Mesh mesh = new Mesh();
        mesh.vertices = baseMesh.vertices;
        mesh.uv = baseMesh.uv;
        ...
        CWeightList[] nodeWeights = new CWeightList[rend.bones.Length];
        ...
        Matrix4x4[] bindPoses = new Matrix4x4[rend.bones.Length];
        for (int i = 0; i < rend.bones.Length; i++)
        {
            if (rend.bones[i].parent != null)
                bindPoses[i] = rend.bones[i].worldToLocalMatrix * rend.bones[i].transform.localToWorldMatrix;
            else
                bindPoses[i] = rend.bones[i].worldToLocalMatrix;
        ...
        {
            Debug.Log(baseMesh.boneWeights.Length);
            BoneWeight bw = baseMesh.boneWeights[i];
            if (bw.weight0 != 0.0f)
            {
                localPt = baseMesh.bindposes[bw.boneIndex0].MultiplyPoint3x4(baseMesh.vertices[i]);
        ...
        //    for (int i = 0; i < newVert.Length; i++)
        //    {
        //        newVert[i] = model.transform.InverseTransformPoint(newVert[i]);
        //    }
